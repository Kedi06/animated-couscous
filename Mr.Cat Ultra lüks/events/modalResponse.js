    ...
    async execute(interaction, client) {
        ...
        if (interaction.customId === 'playModal') {
            ...
            if (musicNameOrLink.includes('https://' || 'http://')) {
                if (!musicNameOrLink.includes('youtube.com' || 'youtu.be' || 'open.spotify.com' || 'spotify.com' || 'https://open.spotify.com/playlist/')) {
            ...
            const message =  await interaction.editReply({ embeds: [musicEmbed] });
            const durationString = lastSong.formattedDuration;
const duration = parseDuration(durationString); // Süreyi ayrıştır

// Süre dolunca mesajı silmek için
setTimeout(() => {
    message.delete();
}, duration * 1000);

// Süreyi ayrıştırmak için yardımcı fonksiyon
